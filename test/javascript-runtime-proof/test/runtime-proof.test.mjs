...
test("pins package, engine, WASM, and import identity", () => {
  ...
  assert.deepEqual(report.package, {
    ...
    lifecycleScripts: [],
    lockIntegrity: "sha512-+7ArUWrc1qCtFLjpNVGI47eGih4TKWg3RSB+FfPFtnfZrKlgvCGUUWbClDalFL7lzNAYH82jrPPHsh3LBkuk7g==",
  });
  assert.deepEqual(report.engine, { name: "quickjs-ng", version: "0.15.1" });
  assert.deepEqual(report.dependencyFiles, {
    verified: 24,
    manifestBytes: 3314,
    manifestSha256: "96f2003a8840057195df2700c53e9e3362522fcc6fb6341400d5ca85f4dc0ec8",
  });
  assert.deepEqual(report.posture, {
    ...
    provenanceUrl: "https://registry.npmjs.org/-/npm/v1/attestations/quickjs-wasi@3.2.0",
    advisoryReviewAsOf: "2026-07-28",
    fixedAdvisories: [
      "CVE-2026-0821",
      "CVE-2026-0822",
      "CVE-2026-1144",
    ...
    noKnownApplicableHighCritical: true,
  });
  assert.deepEqual(report.wasm, {
    sourceBytes: 612319,
    sourceSha256: "078199ef140ec06f18cf7382cca6a39cae638b2d49dca6bdfd139023abb71db4",
    cappedBytes: 612321,
...
test("rejects dependency tampering before runtime child code executes", async () => {
  ...
  try {
    ...
    const dependencyPackageJson = fileURLToPath(new URL(import.meta.resolve("quickjs-wasi/package.json")));
    const dependencyRoot = dirname(dependencyPackageJson);
    const fixtureDependencyRoot = join(fixture, "node_modules", basename(dependencyRoot));
    ...
    const result = spawnSync(process.execPath, [join(fixture, "proof-parent.mjs"), "identity"], {
      encoding: "utf8",
      timeout: 5_000,
      maxBuffer: 64 * 1024,
    });
    assert.notEqual(result.status, 0);
    assert.match(result.stderr, /installed quickjs-wasi files do not match the pinned manifest/);
...
test("exposes only the required restricted guest intrinsic surface", () => {
  ...
  assert.deepEqual(Object.keys(report.forbidden).sort(), [
    "ArrayBuffer",
    "Atomics",
    "BigInt",
    "Buffer",
...
test("contains persistent-object OOM and disposes the child VM cleanly", () => {
  ...
  assert.equal(report.childExitCode, 0);
  assert.equal(report.childSignal, null);
  assert.ok(report.wasmMemoryBytes < 8 * 1024 * 1024, `WASM grew to ${report.wasmMemoryBytes}`);
});
...
test("bounds cancellation-child output before termination", () => {
  ...
  assert.ok(report.stderrBytes <= 64 * 1024);
  assert.equal(report.aliveAfterExit, false);
  assert.ok(report.cleanupMs < 1_500, `output-limit cleanup took ${report.cleanupMs}ms`);
});
...
test("reaps a child that never reaches cancellation readiness", () => {
  ...
  assert.ok(report.cleanupMs < 1_500, `startup-timeout cleanup took ${report.cleanupMs}ms`);
});
...
test("preserves a child exit before cancellation readiness", () => {
  ...
  assert.equal(report.aliveAfterExit, false);
  assert.ok(report.cleanupMs < 1_500, `early-exit cleanup took ${report.cleanupMs}ms`);
});
...
test("terminates an unresolved capability child without leaving an orphan", () => {
  ...
  assert.equal(report.aliveAfterExit, false);
  assert.ok(report.terminationAttempts >= 1);
  assert.ok(report.cleanupMs < 1_500, `cancellation cleanup took ${report.cleanupMs}ms`);
});
...
test("repeated OOM children leave parent memory bounded", () => {
  ...
  assert.ok(report.parentRssGrowthBytes < 16 * 1024 * 1024, `parent RSS grew by ${report.parentRssGrowthBytes}`);
  assert.ok(report.elapsedMs < 20_000, `stress proof took ${report.elapsedMs}ms`);
});
