	...
	const cases: Array<[string, (definition: Record<string, unknown>) => void]> =
		[
			...
			[
				"$.limits.extra",
				(definition) => {
					(definition.limits as Record<string, unknown>).extra = true;
				},
			],
			[
				"$.steps[0].extra",
				(definition) => {
					(
						(definition.steps as Record<string, unknown>[])[0] as Record<
							string,
							unknown
						>
					).extra = true;
...
test("rejects unsafe non-JSON inputs without leaking accidental errors", () => {
	...
	unsafe.push([
		accessor,
		/\$\.evil: object properties must be enumerable data properties/,
	]);
	...
	unsafe.push([
		hostileLength,
		/\$\.steps: array length cannot be safely inspected/,
	]);
	...
	for (const value of [undefined, 1n, () => undefined]) {
		...
		unsafe.push([
			definition,
			/\$\.bad: (?:undefined|bigint|function) is not a JSON value/,
		]);
	}
	...
	unsafe.push([
		nonPlain,
		/\$\.bad: object must be plain or have a null prototype/,
	]);
...
test("rejects duplicate IDs across every node kind", () => {
	...
	const pipeline = (definition.steps as Record<string, unknown>[])[2] as Record<
		string,
		unknown
	>;
	...
	assertInvalid(
		definition,
		/\$\.steps\[2\]\.stages\[0\]\.id: duplicate ID accuracy/,
	);
});
...
test("enforces reference declarations, ordering, parent matching, and local scope", () => {
	...
	const only = (missingArg.steps as Record<string, unknown>[])[0] as Record<
		string,
		unknown
	>;
	only.prompt = {
		template: "{{x}}",
		values: { x: { ref: "arg", name: "missing" } },
	};
	cases.push([missingArg, /argument reference is not declared/]);

	const forward = validDefinition();
	const brief = (forward.steps as Record<string, unknown>[])[0] as Record<
		string,
		unknown
	>;
	brief.prompt = {
		template: "{{x}}",
		values: { x: { ref: "step", stepId: "summary" } },
	};
	cases.push([forward, /step reference must name a prior top-level step/]);

	const sibling = validDefinition();
	const checks = (sibling.steps as Record<string, unknown>[])[1] as Record<
		string,
		unknown
	>;
	const accuracy = (checks.tasks as Record<string, unknown>[])[0] as Record<
		string,
		unknown
	>;
	accuracy.prompt = {
		template: "{{x}}",
		values: { x: { ref: "task", stepId: "checks", taskId: "clarity" } },
	};
	cases.push([
		sibling,
		/task reference must match a task in a prior parallel step/,
	]);

	const wrongParent = validDefinition();
	const summary = (wrongParent.steps as Record<string, unknown>[])[3] as Record<
		string,
		unknown
	>;
	summary.prompt = {
		template: "{{x}}",
		values: { x: { ref: "task", stepId: "brief", taskId: "accuracy" } },
	};
	cases.push([
		wrongParent,
		/task reference must match a task in a prior parallel step/,
	...
	(
		(itemOutside.steps as Record<string, unknown>[])[0] as Record<
			string,
			unknown
		>
	).prompt = {
	...
	const lanes = (firstPrevious.steps as Record<string, unknown>[])[2] as Record<
		string,
		unknown
	>;
	(
		(lanes.stages as Record<string, unknown>[])[0] as Record<string, unknown>
	).prompt = {
		template: "{{x}}",
		values: { x: { ref: "previous" } },
	};
	cases.push([
		firstPrevious,
		/previous reference is only valid after the first pipeline stage/,
	]);

	const pipelineSelf = validDefinition();
	const selfLanes = (
		pipelineSelf.steps as Record<string, unknown>[]
	)[2] as Record<string, unknown>;
	(
		(selfLanes.stages as Record<string, unknown>[])[0] as Record<
			string,
			unknown
		>
	).prompt = {
		template: "{{x}}",
		values: { x: { ref: "step", stepId: "target-lanes" } },
	};
...
test("requires pipeline items to name an array argument and final refs to resolve", () => {
	...
	const pipeline = (wrongItems.steps as Record<string, unknown>[])[2] as Record<
		string,
		unknown
	>;
	...
	assertInvalid(
		wrongItems,
		/pipeline items argument must have an array schema/,
	);
	...
	assertInvalid(
		badFinal,
		/final task reference does not match a parallel task/,
	);
...
test("requires exact template tokens and an equal binding set", () => {
	const cases: Array<[string, Record<string, unknown>, RegExp]> = [
		[
			"missing",
			{ template: "{{x}}", values: {} },
			/missing binding for placeholder x/,
		],
		[
			"unused",
			{ template: "literal", values: { x: { ref: "arg", name: "x" } } },
			/binding is not used/,
		],
		[
			"whitespace",
	...
	(
		(duplicateToken.steps as Record<string, unknown>[])[0] as Record<
			string,
			unknown
		>
	).prompt = {
	...
	(
		(literal.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).prompt = {
		template: "$brief is literal",
		values: {},
	};
...
test("validates the closed embedded schema dialect and semantics", () => {
	...
	stringBounds.args = {
		text: { type: "string", minLength: 65_536, maxLength: 65_536 },
	};
	...
	enumBoundary.args = {
		value: {
			type: "integer",
			enum: Array.from({ length: 256 }, (_, index) => index),
		},
	};
	...
	(
		(structuredArray.steps as Record<string, unknown>[])[0] as Record<
			string,
			unknown
		>
	).output = {
	...
	assertInvalid(
		structuredArray,
		/structured output schema root must be object/,
	);
});
...
test("rejects unsupported discriminants and failure policies", () => {
	...
	(
		(badStep.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).type = "nested";
	assertInvalid(badStep, /unsupported step discriminant/);

	const badOutput = minimalDefinition();
	(
		(badOutput.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).output = { mode: "json" };
	assertInvalid(badOutput, /unsupported output mode/);

	const badPolicy = validDefinition();
	(
		(badPolicy.steps as Record<string, unknown>[])[2] as Record<string, unknown>
	).onFailure = "continue";
...
test("rejects statically guaranteed leaf slots beyond maxCalls", () => {
	...
	assertInvalid(
		over,
		/\$\.limits\.maxCalls: static leaf slots exceed maxCalls/,
	);
});
...
test("enforces identifier, agent, display, and template UTF-8 boundaries", () => {
	...
	const agentStep = (
		agentBoundary.steps as Record<string, unknown>[]
	...
	const displayStep = (
		displayBoundary.steps as Record<string, unknown>[]
	)[0] as Record<string, unknown>;
	...
	const templateStep = (
		templateBoundary.steps as Record<string, unknown>[]
	)[0] as Record<string, unknown>;
...
test("rejects prohibited controls, bidi controls, forbidden names, and unpaired surrogates", () => {
	...
	(
		(control.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).agent = "bad\nagent";
	assertInvalid(control, /agent: contains a prohibited control character/);

	const carriageReturn = minimalDefinition();
	(
		(carriageReturn.steps as Record<string, unknown>[])[0] as Record<
			string,
			unknown
		>
	).prompt = { template: "bad\rprompt", values: {} };
	assertInvalid(
		carriageReturn,
		/template: contains a prohibited control character/,
	);

	const bidi = minimalDefinition();
	(
		(bidi.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).agent = "bad\u202eagent";
...
test("accepts collection caps and rejects cap plus one", () => {
	...
	tasksBoundary.steps = [
		{
			type: "parallel",
			id: "group",
			tasks: Array.from({ length: 256 }, (_, index) => leaf(`t${index}`)),
		},
	];
	...
	(
		(
			(tasksBoundary.steps as Record<string, unknown>[])[0] as Record<
				string,
				unknown
			>
		).tasks as unknown[]
	...
	stagesBoundary.steps = [
		{
			...
			stages: Array.from({ length: 32 }, (_, index) => leaf(`g${index}`)),
			onFailure: "stop-item",
		},
	];
	...
	(
		(
			(stagesBoundary.steps as Record<string, unknown>[])[0] as Record<
				string,
				unknown
			>
		).stages as unknown[]
...
function definitionAtGlobalDepth(depth: 32 | 33): Record<string, unknown> {
	...
	for (let currentDepth = depth - 1; currentDepth >= 2; currentDepth -= 1)
		nested = [nested];
...
test("counts the definition root as depth 1, allowing depth 32 and rejecting depth 33", () => {
	...
	assertInvalid(
		definitionAtGlobalDepth(33),
		/definition nesting depth exceeds 32/,
	);
});
...
function objectSchemaAtBytes(bytes: number): Record<string, unknown> {
	...
	(
		(
			(schema.properties as Record<string, unknown>).value as Record<
				string,
				unknown
			>
		).enum as string[]
	)[0] = "x".repeat(bytes - encoded);
...
test("accepts a 64 KiB schema and rejects one byte more", () => {
	const boundary = minimalDefinition();
	(
		(boundary.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).output = {
		mode: "structured",
		schema: objectSchemaAtBytes(64 * 1024),
	};
	assert.doesNotThrow(() => parseWorkflowDefinition(boundary));

	const over = minimalDefinition();
	(
		(over.steps as Record<string, unknown>[])[0] as Record<string, unknown>
	).output = {
		mode: "structured",
		schema: objectSchemaAtBytes(64 * 1024 + 1),
	};
...
function definitionAtBytes(bytes: number): Record<string, unknown> {
	...
	((args.padlast as Record<string, unknown>).enum as string[])[0] = "x".repeat(
		bytes - before - overhead,
	);
...
test("accepts a 256 KiB canonical definition and rejects one byte more", () => {
	assert.doesNotThrow(() =>
		parseWorkflowDefinition(definitionAtBytes(256 * 1024)),
	);
	assertInvalid(
		definitionAtBytes(256 * 1024 + 1),
		/canonical definition size exceeds 256 KiB/,
	);
});
...
test("stops cloning shared large strings as soon as the definition size cap is exceeded", () => {
	...
	definition.padding = new Proxy(
		Array.from({ length: 1_000 }, () => shared),
		{
			getOwnPropertyDescriptor(target, property) {
				if (
	...
	assert.ok(
		inspectedEntries <= 4,
		`inspected ${inspectedEntries} shared string occurrences`,
	);
});
...
function countEntries(value: unknown): number {
	...
	if (Array.isArray(value))
		return (
			value.length +
			value.reduce((total, item) => total + countEntries(item), 0)
		);
	return (
		Object.keys(value).length +
		Object.values(value).reduce((total, item) => total + countEntries(item), 0)
	);
}
	...
	for (let index = 0; index < contributions.length; index += 1) {
		...
		args[`fill${index}`] =
			contribution === 2
				...
				: {
						type: "string",
						enum: Array.from({ length: contribution - 3 }, (_, enumIndex) =>
							String(enumIndex),
						),
					};
	}
...
function definitionWithIds(idCount: number): Record<string, unknown> {
	...
	while (remainingTasks > 1) {
		...
		const tasks = Array.from({ length: taskCount }, () =>
			leaf(`node${taskNumber++}`),
		);
		steps.push({ type: "parallel", id: groupId, tasks });
		remainingTasks -= taskCount + 1;
	}
	if (remainingTasks === 1)
		steps.push({ type: "agent", ...leaf(`node${taskNumber++}`) });
	definition.steps = steps;
	definition.result = {
		ref: "step",
		stepId: (steps.at(-1) as Record<string, unknown>).id,
	};
	return definition;
}
